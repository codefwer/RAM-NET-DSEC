...
class BaseTrainer:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            result = self._train_epoch(epoch)  #result是一个字典
            ...
            if result['loss'] < init_loss:
                init_loss = result['loss']
                self._save_checkpoint(epoch, log, save_best=True,init_loss=init_loss)
            ...
            print('epoch: {} average batch loss is: {}'.format(epoch, result['loss']))
            print('epoch: {} every alone loss in batch is: {}'.format(epoch, result['losses']))
    ...
    def _save_checkpoint(self, epoch, log, save_best=False,init_loss=float('inf')):
        """
        Saving checkpoints
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        self.init_loss = checkpoint['init_loss']
        if self.with_cuda:
